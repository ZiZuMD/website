...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $servername = "127.0.0.1:3307";  // Имя сервера MySQL
    $username = "root"; // Имя пользователя MySQL
    $db_password = "";      // Пароль пользователя MySQL
    $dbname = "users";  // Имя базы данных MySQL
    ...
    $sql = "INSERT INTO users (email, password, name) VALUES ('$email', '$formPassword', '$name')";

    if ($conn->query($sql) === TRUE) {
        // Перенаправление на другую страницу после успешной обработки
        header("Location: form-authorization.html");
        exit;
    } else {
        ...
        <main class="content__main">
          ...
          <form class="form" action="#" method="post" autocomplete="off">
            <div class="form__row">
              ...
              <input class="form__input form__input--error" type="email" name="email" id="email" required value="" placeholder="Введите e-mail">
